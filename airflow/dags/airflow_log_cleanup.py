...
with DefaultDAG(
    dag_id=os.path.basename(__file__).replace(".pyc", "").replace(".py", ""),
    schedule="@hourly",
    start_date=pendulum.datetime(2023, 12, 2, tz="Asia/Jakarta"),
    catchup=True,
    ...
    DEFAULT_MAX_LOG_AGE_IN_DAYS = int(
        Variable.get("airflow_log_cleanup__max_log_age_in_days", 2)
    )
    ENABLE_DELETE = True
    NUMBER_OF_WORKERS = 1
    DIRECTORIES_TO_DELETE = [BASE_LOG_FOLDER]
    ...
    LOG_CLEANUP_PROCESS_LOCK_FILE = "/tmp/airflow_log_cleanup_worker.lock"
    logging.info("ENABLE_DELETE_CHILD_LOG  %s", {ENABLE_DELETE_CHILD_LOG})
