...
logger: Logger for logging messages.  
Functionality:  
Moves a maximum of 35 UDR files (sorted by *sort_by_modified_time()* function in ascending order) from the RAW to the PROC directory.
...
Functionality:  
Converts the timestamp to the format %Y-%m-%d %H:%M:%S. This function is used with *_proc_normal()* and *_proc_suspend()* function.
...
If the nanoseconds portion of *lastpolledtime* is empty (''), it updates the dictionary's ns value to 0. This prevents issues with database constraints, which require *ns* to be non-null.
